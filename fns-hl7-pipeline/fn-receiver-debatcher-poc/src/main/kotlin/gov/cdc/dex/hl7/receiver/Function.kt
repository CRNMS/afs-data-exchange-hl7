...
class Function {
    companion object {
        ...
        const val MSH_9_PATH = "MSH-9" // not used
        const val MSH_21_1_1_PATH = "MSH-21[1].1"  // not used
    ...
    fun eventHubProcessor(
        ...
        if (messages != null) {
            ...
            for (message in messages) {
                ...
                var token: Token = event.claimToken
                if ( event.eventType == BLOB_CREATED) {
                    ...
                    if(!isValidMessage){
                        ...
                        val (metadata, summary) = buildMetadata(token.STATUS_ERROR, eventHubMD[nbrOfMessages], startTime, provenance, "Message missing required Meta Data.", token)
                    ...
                    BufferedReader(reader).use { br ->
                        br.forEachLine { line ->
                            val lineClean = line.trim().let { if ( it.startsWith(token.UTF_BOM) )  it.substring(1)  else it}
                            if ( lineClean.startsWith("FHS") || lineClean.startsWith("BHS") || lineClean.startsWith("BTS") || lineClean.startsWith(("FTS")) ) {
                                ...
                                if ( lineClean.startsWith("MSH") ) {
                                    ...
                                    if ( mshCount > 1 ) {
                                        ...
                                        val (metadata, summary) = buildMetadata(token.STATUS_SUCCESS, eventHubMD[nbrOfMessages], startTime, provenance, null, token)
                    ...
                    if (mshCount > 0) {
                        ...
                        val (metadata, summary) = buildMetadata(token.STATUS_ERROR, eventHubMD[nbrOfMessages], startTime, provenance, "No valid message found.", token)
                        // send empty array as message content when content is invalid
                        prepareAndSend(arrayListOf(), DexMessageInfo(null, null, null, null, HL7MessageType.valueOf(messageType)), metadata, summary, evHubSender, evHubErrsName, context)
                    }
                } // .if
    ...
    private fun getMessageInfo(metaDataMap: Map<String, String>, mmgUtil: MmgUtil, message: String, token: Token): DexMessageInfo {
        val eventCode = extractValue(message, token.EVENT_CODE_PATH)
        ...
        val msh21Gen = extractValue(message, token.MSH_21_2_1_PATH)
        val msh21Cond = extractValue(message, token.MSH_21_3_1_PATH)

        var jurisdictionCode = extractValue(message, token.JURISDICTION_CODE_PATH)
        if (jurisdictionCode.isEmpty()) {
            jurisdictionCode = extractValue(message, token.ALT_JURISDICTION_CODE_PATH)
        }
    ...
    private fun buildMetadata (status: String, eventHubMD: EventHubMetadata, startTime: String, provenance: Provenance, errorMessage: String? = null, token: Token) : Pair<DexMetadata, SummaryInfo> {
        val processMD = ReceiverProcessMetadata(status, eventHubMD)
        processMD.startProcessTime = startTime
        processMD.endProcessTime = Date().toIsoString()
        var summary = SummaryInfo("RECEIVED")
        if (status == token.STATUS_ERROR) {
